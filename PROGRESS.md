...
- **Forenklet meny:** "Arbeid" var det samme som "Forside" og "Kontakt" det samme som "Om meg" — begge kuttet. Meny-knappen og hele `MenuOverlay`-komponenten er fjernet; i stedet ligger nå "om meg" som en glass-pille rett ved siden av spiral/liste-toggelen i topbaren (ny `.centerNav`-klynge, `.navLink` matcher togglens materiale). `om meg` peker til `/om` (siden bygges i Fase 3 — 404 inntil da).
- **Liste-kortet roterer med musen:** preview-kortet i liste-visningen lener seg nå etter *horisontal* musefart i stedet for en tilfeldig vinkel per rad. Hver piksel sidelengs gir `TILT_PER_PX = 0.45°` (maks ±7°), og en rAF eased vinkelen tilbake mot 0 (`TILT_DECAY = 0.86`/frame) når pekeren hviler — svak, men merkbar. `--tilt` settes per frame fra JS, så `rotate` er tatt ut av CSS-transitionen for å ikke dobbelt-glatte. `prefers-reduced-motion` → ingen rAF, kortet står flatt.
