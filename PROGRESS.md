...
- **Kontakt-seksjon (`#kontakt`):** overskrift + «Basert i Bergen»-tekst er på plass, men e-post og sosiale lenker mangler — `EMAIL`/`SOCIALS` står tomme øverst i `About.tsx` og rendres kun når de fylles inn (ingen falske placeholder-lenker). **Venter på:** e-postadresse + ev. Instagram/LinkedIn/andre fra Martin.
