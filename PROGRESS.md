...
- **Disiplin-glow på verktøy-taggene:** hvert program har nå en farget glow som viser hvilket område det brukes i — design (lime = sidens `--accent`), bilde (lilla, for Photoshop/Lightroom), video (turkis/cyan) og 3d (amber) — med en liten fargeforklaring (legend) over taggene. La til **Figma** under «bruker daglig».
- **Checkpoints på om-siden:** progresjonsrailen på om-meg-siden har nå én checkpoint per seksjon (intro, verktøy, kontakt), akkurat som CV-siden — seksjonene fikk id-er (`om-intro`, `om-verktoy`, `kontakt`) som mates til `ScrollProgress`.
- **Ekte annonser i galleriet:** alle fire galleri-slottene på Sporty-skjermer-siden viser nå ekte annonser — Auto 8-8 (den tidligere ubrukte `sporty-skjermer.webp`), EiendomsMegler 1 (Olderkjerret 3), Vancowill AS og Juliussen Trafikkskole. De tre nye ble skalert ned fra 3840×2160 og konvertert til webp (1600×900) via `sharp`. Ingen placeholder-rammer igjen; galleri-introen er oppdatert deretter.
- **Bildeforhold-fiks:** galleri-bildene bruker nå `width:100%; height:auto` (kilden er ekte 16:9) + `align-items:start` på grid-en, etter at `aspect-ratio`+`object-fit:cover` feilaktig strakk Auto 8-8-bildet høyt.
- Verifisert: `npm run build` ✅ (12/12 sider), eslint rent.

### Verktøy-seksjon på om-siden (runde 39)
- **La til en «verktøy»-seksjon på om-meg-siden** (mellom intro og kontakt) som viser programmene Martin jobber i, gruppert i tre nivåer som CV-en: «bruker daglig» (Lightroom, Photoshop, Illustrator, DaVinci Resolve), «også» (Premiere Pro, After Effects) og «har brukt» (Blender, Unreal Engine). Programmene vises som små statiske pill-tags i sidens stil (ikke klikkbare, så ingen hover-affordance). Responsivt: label-kolonnen stables over taggene under 720px.
- Verifisert: `npm run build` ✅, eslint rent.

### «Auto 8-8» → «Sporty-skjermer» + dedikert case-side (runde 38)
- **Erstattet prosjektet «Auto 8-8 — Eide Bruktbil» med «Sporty-skjermer»** i `lib/projects.ts`: ny slug `sporty-skjermer`, tag «skjermreklame», ny blå gradient. Siden handler ikke om én spesifikk annonse, men om flere annonser laget og publisert på skjermene på Sporty-sentrene (både Sportys egne budskap og kampanjer for eksterne annonsører).
- **Nytt kortbilde:** konverterte et ekte skjermbilde fra senteret (knespesialistene.no-annonsen, `P1088727.jpg` fra Martins OneDrive-mappe) til `public/projects/sporty-skjermer-card.webp` (1600×901, webp, via `sharp`). Den gamle bildefila ble *ikke* overskrevet — den ligger igjen som `sporty-skjermer.webp` (ubrukt, kan slettes senere).
- **Dedikert case-side** `components/cases/SportySkjermer.tsx` (+ `.module.css`), registrert i `CASES`-oppslaget. Bygd som en showcase (ikke prosess-tidslinje): hero med skjermbildet, meta (rolle/år/kunde: Sporty + eksterne annonsører/flate), ingress, seksjoner om «flaten» (lest på sekunder, på avstand, uten lyd) og «arbeidet» (tar utgangspunkt i annonsørens identitet, definerer budskap/utforming, leverer godkjent annonse), ett fremhevet ekte eksempelbilde, og et galleri med placeholder-rammer (samme dashed-frame-stil som «Fra sofa til Sporty») som byttes ut når de andre annonsebildene er klare.
- **Gjenstår:** de andre annonsebildene (finnes som flere skjermfotos i samme OneDrive-mappe: Horde «Rentesvett?», HARDDISK «Lei av å trene?», Mostøl/protein m.m.) kan legges inn i galleriet — bare å bytte placeholder-figurene mot `<Image>`.
- Verifisert: `npm run build` ✅ (alle prosjektsider genereres), eslint rent.

### Progresjonsrail som scrollbar overalt + ytelsesfiks + CV-nav (runde 37)
- **Reverserte den egne dragbare scrollbaren (runde 36) — den gjorde sida treig.** `ScrollBar` la en `ResizeObserver` på `body` pluss en global scroll-lytter som leste `scrollHeight`/layout hver gang (tvungne reflows), montert på *alle* sider inkl. den animasjonstunge forsiden — det ga merkbar hakking. Komponenten + CSS-en er slettet, og `window.__lenis`-koblingen i `SmoothScroll` er fjernet igjen.
- **Progresjonsraila (CV-stilen) er nå scrollbar-erstatningen på alle scrollesider.** Den native baren holdes fortsatt skjult i `globals.css` (så ingenting dyttes), og `ScrollProgress` står nå på prosjektsidene og om-meg-siden i tillegg til CV-en. Den er lett: kun en rAF-throttlet scroll-lytter, ingen `ResizeObserver` og ingen løpende reflow.
- **`ScrollProgress` gjort generell:** `sections` er nå valgfri. Med seksjoner (CV) får man checkpoint-noder som før; uten (om / prosjekt) blir det bare den lime fremdrifts-fyllingen — samme visuelle språk, uten noder. Raila skjuler seg selv (`data-show=false` → `opacity:0`) på sider som ikke kan scrolles, så forsiden/korte sider ikke får et tomt spor.
- **Nav-highlight på CV fikset:** `/cv` regnes nå som «om meg» i `TopNav` (CV-en er lenket fra om-siden og hører konseptuelt under den), i stedet for å falle tilbake til «forside».
- Verifisert: `npm run build` ✅, eslint rent.

### Egendefinert overlay-scrollbar (runde 36 — reversert i runde 37)
- **Native scrollbar skjult på tvers av nettlesere, og erstattet med en egen overlay-scrollbar** i samme lime-på-dim-språk som progresjonsraila. Den native baren skjules helt i `globals.css` (`scrollbar-width: none` for Firefox, `-ms-overflow-style: none` for gammel Edge, og `::-webkit-scrollbar { display: none }` for Chrome/Safari/Edge) — viktig poeng: ved å *skjule* den (ikke restyle `::-webkit-scrollbar`) reserverer den aldri bredde, så innholdet blir aldri dyttet når en side blir scrollbar.
- **Ny komponent `ScrollBar.tsx` (+ `.module.css`)**, montert én gang i root-layouten så den lever på tvers av rutebytter. Den er et vanlig DOM-element (ikke nettleser-chrome), så den ser og oppfører seg likt i Chrome, Safari, Firefox og Edge. En slank lime-tommel ligger flush mot høyre kant: høyden er forholdet viewport/innhold, og posisjonen følger `scrollY` (rAF-throttlet scroll-lytter). Tommelen kan dras, og klikk på sporet hopper dit. Den vokser/lyser opp på hover og under dragging.
- **Spiller på lag med Lenis:** `SmoothScroll` legger nå Lenis-instansen på `window.__lenis`, og scrollbaren ruter drag-til-scroll gjennom Lenis når den finnes (under-sidene) i stedet for `window.scrollTo` — ellers ville draget kjempet mot smooth-scroll-motoren. På forsidens listevisning (uten Lenis) faller den tilbake til vanlig vindu-scroll.
- **Skjuler seg selv** når sida ikke kan scrolles (`data-show=false` → `opacity:0` + `pointer-events:none`), så forsidens faste lerret ikke får en død kant-stripe. En `ResizeObserver` på `body` fanger høyde-endringer fra rutebytter/bildelasting. Skjult på touch (`pointer: coarse`) der OS-en håndterer dette selv og en kant-stripe ville stjålet kant-swipes. Dekorativ (`aria-hidden`); tastatur-scroll (piltaster/space/PageUp/Down) er uendret siden native scroll aldri er deaktivert.
- Verifisert: `npm run build` ✅ (alle 12 sider), eslint rent.
