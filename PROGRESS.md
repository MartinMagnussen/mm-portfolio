...
- **Highlightede kort holdes innenfor viewporten (mobil-fiks):** på smale skjermer svingte front-kortene delvis ut av skjermkanten. Front-banens svingutslag (`ampX`) kappes nå mot `vw/2 − halv kortbredde·1.06 − 14px`, målt fra det bredeste (featured) kortet. På telefon krymper svingen mot null så et highlightet kort aldri spiller utenfor; på desktop er taket høyere enn dagens 360px, så ingen synlig endring der. Retur-bane-kortene er bevisst *ikke* begrenset (de skal kunne drive lenger ut). Kortbredde måles ved mount + på resize (utenfor render-loopen).
- **Mykt/dempet scroll for desktop (treghet):** musehjul mater nå en hastighet som blør inn i målet og decay-er (`WHEEL_FRICTION = 0.85`), så transportbåndet glir mykt til ro i stedet for å hakke. `WHEEL_IMPULSE = 1 − friction` gjør at total scroll-distanse er uendret — kun jevnere. Touch (mobil/iPad) skriver fortsatt målet direkte og er helt upåvirket. `prefers-reduced-motion` hopper over tregheten. (Valgte lett egenutviklet treghet framfor GSAP ScrollSmoother, som er en betalt Club-plugin og uansett er laget for ekte side-scroll, ikke dette faste lerretet.)
