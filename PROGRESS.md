...
- **Kontakt-seksjon (`#kontakt`):** overskrift + «Basert i Bergen»-tekst, e-post-CTA (`martin.magnussen@outlook.com`, `mailto:`) og LinkedIn-lenke (ren URL uten utm-sporeparametre). `EMAIL`/`SOCIALS` ligger øverst i `About.tsx` og rendres kun når de er fylt inn. Ingen telefon (etter ønske).
